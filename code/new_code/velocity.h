...
Vector3 calculateLegVelocity(Vector3 leg_pos, double robot_center_velocity, double robot_arc_radius)
{
    double omega = robot_center_velocity / robot_arc_radius;
    // printf("omega = %.30f\n", omega);
    Vector3 leg_velocity;

    leg_velocity.data[X] = -omega * leg_pos.data[Y];
    leg_velocity.data[Y] = (omega * (leg_pos.data[X] + robot_arc_radius)); //- robot_center_velocity;
    // leg_velocity.data[Y] = (omega * leg_pos.data[X]) + robot_center_velocity;
    leg_velocity.data[Z] = 0;
    // printf("omega razy cos = %.20f\n", (omega * (leg_pos.data[X] + robot_arc_radius)));
    ...
    // leg_radius.data[X] = leg_pos.data[X] + robot_arc_radius;
    // leg_radius.data[Y] = leg_pos.data[Y];
    // leg_radius.data[Z] = 0;
