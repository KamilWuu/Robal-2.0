...
Matrix3 createJakobian(double q1, double q2, double q3, double l1, double l2, double l3, double k)
{ // kąty w radianach
...
void setServosAngles(LegPosition leg, double q1, double q2, double q3)
{ // angles in radians
    RobotSide side;
...
Vector3 getAnglesRobotCenter(LegPosition pos, RobotSide side, Vector3 global_pos)
{
    ...
    if (checkPosition(pos, x, y, z))
    {
        ...
        leg_pos.data[1] = yp;
        leg_pos.data[2] = zp;
        angles = calculateInvertedKinematics(leg_pos, side);
    }
    return angles;
}
...
int main()
{
    Matrix3 jacobian, inversed_jacobian, inversed_jacobian_2;
    Vector3 d_positions, d_angles;

    Vector3 start_angles, start_pos;
    RobotSide leg_side;
    LegPosition leg = RIGHT_MIDDLE;
    ...
    double delta_time = 0.1; // Czas w sekundach na aktualizację kątów
    double total_time = 10;  // Całkowity czas ruchu w sekundach
    ...
    inversed_jacobian = createInversedJacobian(start_angles);
    printf("Jakobian odwrotny (z wyznaczonych wzorów)::\n");
